---
name: st-note-check
description: Consolidate Subject Tutoring student progress notes. Pulls session data from Appointment-Plus, cross-references Google Docs notes and grade trackers, flags missing notes, grade drops, and concerns. Outputs a Google Sheet and Google Doc report.
user-invocable: true
allowed-tools:
  ...
  - mcp__google-workspace__readGoogleDoc
  - mcp__google-workspace__listDocumentTabs
  - mcp__google-workspace__getGoogleDocContent
  - mcp__google-workspace__createGoogleSheet
  - mcp__google-workspace__updateGoogleSheet
  - mcp__google-workspace__appendSpreadsheetRows
  - mcp__google-workspace__addSheet
  - mcp__google-workspace__createGoogleDoc
  - mcp__google-workspace__insertText
  - mcp__google-workspace__listFolder
  - Bash
  - Read
---
...
Consolidates student progress notes for Subject Tutoring sessions. Pulls session data from Appointment-Plus, reads each student's Google Doc (grade tracker + session notes), and produces a Google Sheet + Google Doc report with flags and recommendations.
...
2. If redirected to a login page (URL contains `login.php`), **sign in automatically**:
   a. Read credentials from the `.env` file in the project root (`C:\projects\hlc-agents\.env`) using the `Read` tool. Parse `AP_USERNAME` and `AP_PASSWORD` values.
   b. Take a snapshot to identify the login form fields.
   c. Fill in the username and password fields using `browser_fill_form` or `browser_type`.
   d. Click the login/submit button.
   e. Wait for navigation to complete, then navigate back to the reports URL above.
   f. If login fails (still on login page after submit), inform the user that the stored credentials may be invalid.
...
- If A+ is not logged in (redirect to login.php), sign in automatically using credentials from `.env`. If login fails, ask the user to verify their credentials.
